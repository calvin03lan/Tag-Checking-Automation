...
## URL 样式全局选项
- `url_style_options.py` 负责 URL title / language segment 的全局选项持久化（跨任务复用）。
- 选项支持 `pws/cms` 分别配置，并可针对 `tc/sc/en` 独立设置语言片段。
...
- URL 拼接默认规则：`pws = https://www.hangseng.com + lang + path`；`cms = https://cms.hangseng.com + path + lang + /index.html`。
